...
"""
The following script extracts x-vectors from input recordings and generates the corresponding ground truth labels.

@Authors: Mireia Diez, Lukas Burget, Federico Landini, Jan Profant
@Emails: mireia@fit.vutbr.cz, burget@fit.vutbr.cz, landini@fit.vutbr.cz, jan.profant@phonexia.com
"""
...
from utils.rttm_utils import rttm_to_hard_labels
from diarization_lib import mkdir_p
...
if __name__ == '__main__':
    ...
    parser.add_argument('--in-rttm-dir', required=False, type=str,
                        help='Input directory with reference rttm files')
    parser.add_argument('--out-xvector-label-GT-dir', required=True, type=str,
                        help='Output directory to save x-vector ground truth labels, obtained from reference rttm files')
            ...
            with open(args.out_ark_fn, 'wb') as ark_file:
                for fn in file_names:
                    with Timer(f'Processing file {fn}'):
                        ...
                        # for the labels
                        y_labels_fr = rttm_to_hard_labels(args.in_rttm_dir + fn + ".rttm", precision=samplerate)
                        my_y_list = []

                        for segnum in range(len(labs)):
                            seg_lab = y_labels_fr[0][labs[segnum, 0]:labs[segnum, 1]]
                            ...
                            if seg.shape[0] > 0.01 * samplerate:  # process segment only if longer than 0.01s
                                ...
                                seg_lab = np.r_[
                                    seg_lab[noverlap // 2 - 1::-1], seg_lab, seg_lab[-1:-winlen // 2 - 1:-1]]
                                ...
                                for start in range(0, slen - seg_len, seg_jump):
                                    ...
                                    if np.isnan(xvector).any():
                                        ...
                                        y_labels_xvec = seg_lab[start * (winlen - noverlap):start * (
                                                    winlen - noverlap) + seg_len * (winlen - noverlap) + noverlap]
                                        my_y_list.append(np.sum(y_labels_xvec, axis=0) / len(y_labels_xvec))
                                ...
                                if slen - start - seg_jump >= 10:
                                    ...
                                    if np.isnan(xvector).any():
                                        ...
                                        y_labels_xvec = seg_lab[(start + seg_jump) * (winlen - noverlap):]
                                        my_y_list.append(np.sum(y_labels_xvec, axis=0) / len(y_labels_xvec))
                                        seg_start = round(
                    ...
                    # Generate GT labels.
                    y_labels = np.array(my_y_list)
                    mkdir_p(args.out_xvector_label_GT_dir)
                    with open(os.path.join(args.out_xvector_label_GT_dir, f'{fn}.init_labs'), 'w') as fp:
                        np.savetxt(fp, y_labels)
